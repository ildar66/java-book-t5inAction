...
import com.tapestry5book.entities.*;
...
public class DemoDataParser {
	...
	public class DemoData {
		...
		public DemoData(Blog blog, List<Article> articles, List<User> users,
				List<Tag> tags) {
			this.blog = blog;
	...
	public DemoData parse(URL resource) {
		...
		return new DemoData(handler.getBlog(), handler.getArticles(),
				handler.getUsers(), handler.getTags());
	}
	...
	private class Handler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName,
			...
			if (localName.equals("blog")) {
				...
				article.setPublishDate(parseDate(attributes
				...
				user.setGender(Enum.valueOf(Gender.class,
						attributes.getValue("gender").toUpperCase()));

				final Date birthday;
				try {
					birthday = DATE_FORMAT.parse(attributes
							.getValue("birthday"));
				...
				user.setRememberMe(Boolean.valueOf(attributes
						.getValue("rememberMe")));
